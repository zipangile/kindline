      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {programmes.map((programme, index) => (
            <Card
              key={index}
              className={`text-left border-t-4 ${programme.color} shadow-lg hover:shadow-2xl transition-all duration-300 bg-white group`}
            >
              <CardHeader className="pb-4">
                <div className="mb-6 p-4 rounded-2xl bg-gray-50 w-fit group-hover:scale-110 transition-transform duration-300">
                  {programme.icon}
                </div>
                <CardTitle className="text-2xl font-bold mb-3 group-hover:text-brand-blue transition-colors">
                  {programme.title}
                </CardTitle>
              ...
              <CardFooter>
                <Button
                  variant="link"
                  className="px-0 text-brand-blue font-bold text-lg hover:no-underline group-hover:translate-x-2 transition-transform"
                  asChild
                >
