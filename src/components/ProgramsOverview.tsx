  ...
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Our Programmes</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {programmes.map((programme, index) => (
            <Card key={index} className="text-left border-none shadow-md hover:shadow-lg transition-shadow bg-gray-50">
              <CardHeader>
                <div className="mb-4">{programme.icon}</div>
                <CardTitle className="text-xl font-bold mb-1">{programme.title}</CardTitle>
                <p className="text-sm font-medium text-blue-600 mb-2">{programme.subtitle}</p>
              </CardHeader>
              <CardContent>
