...
export async function POST(request: Request) {
  try {
    ...
    const signatureKey =
      process.env.LENCO_SIGNATURE_KEY || settings?.lencoSignatureKey;
    ...
    console.log(
      `[Lenco Webhook] Signature verification: derived=${hash}, header=${signature}`,
    );
    ...
    if (event === "collection.successful") {
      const { amount, currency, reference, mobileMoneyDetails, customer } =
        data;
      ...
      if (!existingDonation) {
        ...
        console.log(
          `[Lenco Webhook] Donation created for reference: ${reference}`,
        );
        ...
        if (finalDonation && finalDonation.donorEmail !== "unknown@email.com") {
          sendDonationEmails({
            ...
            console.error("[Lenco Webhook] Email sending failed:", e),
          );
          ...
          await prisma.subscriber
            ...
            .catch((e) =>
              console.error("[Lenco Webhook] Subscriber upsert failed:", e),
            );
        }
      } else {
        console.log(
          `[Lenco Webhook] Donation already exists for reference: ${reference}`,
        );
      }
    }
