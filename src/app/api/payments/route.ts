...
export async function POST(request: Request) {
  try {
    ...
    const secretKey =
      settings?.flutterwaveSecret || process.env.FLUTTERWAVE_SECRET_KEY;
    const publicKey =
    ...
    if (status === "successful") {
      ...
      try {
        ...
        const verificationData = await flw.Transaction.verify({
          id: transaction_id,
        });
        ...
        if (
          ...
          const { amount, currency, customer, meta, tx_ref, id } =
        ...
        console.error(
          "Flutterwave v3 verification failed, attempting direct API call:",
          v3Error,
        );
          ...
          if (!existingDonation) {
            const donation = await prisma.donation.create({
              data: {
                ...
                supabaseUserId: meta?.supabaseUserId || null,
                type:
                  tx_ref && tx_ref.includes("monthly") ? "monthly" : "one-time",
              },
            });
