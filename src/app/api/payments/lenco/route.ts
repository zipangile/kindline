...
export async function POST(request: Request) {
  try {
    ...
    // Security: Validate supabaseUserId if present
    if (supabaseUserId && (typeof supabaseUserId !== 'string' || supabaseUserId.length > SECURITY_LIMITS.SUPABASE_USER_ID || !/^[a-zA-Z0-9-]+$/.test(supabaseUserId))) {
      console.error('[Lenco API] Invalid supabaseUserId format:', supabaseUserId);
      return NextResponse.json({ error: 'Invalid user ID' }, { status: 400 });
    }

    // Security: Validate phone if present
