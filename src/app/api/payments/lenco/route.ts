...
export async function POST(request: Request) {
  try {
    ...
    // Security: Validate supabaseUserId to prevent injection
    if (supabaseUserId && (typeof supabaseUserId !== 'string' || supabaseUserId.length > SECURITY_LIMITS.SUPABASE_USER_ID || !/^[a-zA-Z0-9_-]+$/.test(supabaseUserId))) {
