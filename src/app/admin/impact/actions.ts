...
export async function updateImpactStat(id: string, formData: FormData) {
  await checkAdmin('CONTENT_EDITOR');
  if (!isValidId(id)) throw new Error('Invalid impact stat ID');
  const label = (formData.get('label') as string || '').trim();
  const value = (formData.get('value') as string || '').trim();
  const description = (formData.get('description') as string || '').trim();
  const icon = (formData.get('icon') as string || '').trim();
...
export async function createImpactStory(formData: FormData) {
  ...
  if (author.length > SECURITY_LIMITS.AUTHOR) throw new Error('Author is too long');
  if (authorRole.length > SECURITY_LIMITS.AUTHOR_ROLE) throw new Error('Author role is too long');
...
export async function deleteImpactStory(id: string) {
  await checkAdmin('CONTENT_EDITOR');
  if (!isValidId(id)) throw new Error('Invalid impact story ID');
