...
export async function createImpactStory(formData: FormData) {
  ...
  if (author.length > SECURITY_LIMITS.NAME) throw new Error('Author name is too long');
  if (authorRole.length > SECURITY_LIMITS.LABEL) throw new Error('Author role is too long');
