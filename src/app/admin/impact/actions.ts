...
export async function createImpactStory(formData: FormData) {
  ...
  if (author.length > SECURITY_LIMITS.CATEGORY) throw new Error('Author is too long');
  if (authorRole.length > SECURITY_LIMITS.CATEGORY) throw new Error('Author role is too long');
