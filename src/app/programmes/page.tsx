    ...
    <div className="bg-white">
      ...
      {livePrograms.length > 0 ? (
        livePrograms.map((programme, index) => (
          <section key={programme.id} className={`py-20 ${index % 2 === 1 ? 'bg-gray-50' : ''}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className={`flex flex-col lg:flex-row gap-12 items-center ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                <div className="flex-1">
                  <span className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full ${getBadgeColor(programme.category)}`}>
                    {programme.category}
                  </span>
                  <h2 className="text-3xl font-bold text-gray-900 mt-4 mb-6">{programme.title}</h2>
        ...
        <section className="py-20 text-center">
          <p className="text-gray-500 italic">No active programmes found. Please check back soon.</p>
        </section>
      )}
    </div>
  );
